...
4. **Phase 5** (Issue #69, **implemented 2026-04-27**): insert query normalization at Stage 1 entry. `prepare_match_query` gains a `&QueryNormalizationConfig` parameter; the runtime default is **all-on** (NFKC → ASCII lowercase → whitespace collapse) so indexing and querying agree on folding. `pre_phase_5_disabled()` is the explicit opt-out and is also the serde-default for `BaselineSnapshot.normalization`, letting pre-#69 baselines round-trip under the behaviour they were captured with. AC verification on the new `variant_notation` fixture (21 queries with full-width Latin/digits/punct + ideographic spaces): `recall@5` / `recall@10` / `mrr@10` identical between ON and OFF (the cross-encoder reranker absorbs Stage 1 drift), `ndcg@10` 0.8931 (ON) vs 0.8929 (OFF) — "baseline 以上" satisfied.
   - **Default posture flipped from Phase 2 sketch**: ADR Phase 2 wording said "Default off; opt-in via config to preserve current behaviour." Implementation reversed this — a divergent index/query folding posture would make the FTS path silently inconsistent, so all-on default is safer than opt-in. Pre-#69 callers migrate via the explicit `pre_phase_5_disabled()` helper.
   - **Synonym dictionary hook**: scope shrunk to "interface only" per the issue. No production dictionary lands in #69; downstream owns the mapping.
...
7. **Chunk-level retrieval follow-up** (Issue #76, prerequisite for non-vacuous aggregation evaluation, **implemented 2026-04-27**): extend `ChunkedEmbedding` with `chunk_id` metadata, switch the reference pipeline to chunk-level indexing, add the parent-child helper, and capture per-strategy baselines that reflect actual ranking changes. Touches `recall`/`sae`/`yomu` schema, so it ships under its own issue rather than #67.
   - **Wiring delivered**: `ChunkedEmbedding.chunk_ids: Vec<String>` (`"c0"`, `"c1"`, … auto-populated by `ChunkedEmbedding::new`), `Candidate.chunk_id: Option<String>`, `MergedHit.chunk_id: Option<String>` (serde-default `None` so pre-#76 baseline.json round-trips). Stage 2 `WeightedRrf` fusion key widened from `doc_id` to `(doc_id, chunk_id)` — without this widening every aggregator stays structurally identical to identity (the fusion would re-collapse the chunks Stage 1 just split).
   - **Aggregator semantics**: `IdentityAggregator` preserves chunk-level identity; `MaxChunkAggregator` / `DedupeAggregator` / `TopKAverageAggregator` collapse to parent and emit `chunk_id = None` so Stage 4 sees parent-granular hits. This is the change that makes Identity ≠ MaxChunk on the eval baseline.
   - **Public helper**: `retrieval::group_by_parent(hits) -> HashMap<&str, Vec<&MergedHit>>` for downstream `Aggregator` impls and Stage 4 wiring that needs parent-bucketed chunks.
   - **Pipeline shape**: `vec_docs` is chunk-granular (`embedding, +doc_id, +chunk_id` row per chunk); `docs_fts` and `documents` stay parent-granular because chunk text is not retained on `ChunkedEmbedding`. Vector hits carry `chunk_id = Some(_)`, FTS hits carry `chunk_id = None` — they fuse separately under the new key, and aggregators collapse to parent on their own contract.
   - **`apply_reranker` parent-text posture**: the reranker continues to receive each hit's parent doc body via `corpus_index`. This realises Issue #76 scope 3's "周辺文脈込みの parent text" by default — sibling chunks of the same parent map to the same body, giving the reranker full document context regardless of which chunk surfaced. `chunk_id` is preserved through rerank so chunk-level Identity output keeps its child identity in the final ranking. A configurable `chunk + window` mode is deferred to Phase 3.5 because chunk text is not on `ChunkedEmbedding` (would require a wider extension).
   - **Schema bump**: `BASELINE_SCHEMA_VERSION` `1.0` → `1.1`. The new fusion key splits FTS / Vector contributions for the same parent doc even on single-chunk fixtures, so pre-1.1 baseline.json files would surface as opaque per-metric drift; the bump routes them through the existing schema-mismatch exit so operators see "regenerate the baseline before verifying" instead.
   - **Baseline regeneration**: requires MLX (Apple Silicon) — operator hands off via `eval_harness capture-baseline aggregation={identity|max-chunk|dedupe|topk-average} output=...` and `compare-baselines paths=...` to confirm Identity ≠ MaxChunk divergence on the regenerated set.
