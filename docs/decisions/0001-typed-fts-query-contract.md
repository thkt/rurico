...
> **Correction ([#297](https://github.com/thkt/rurico/issues/297), 2026-09-20)**: The 2026-06-24 note above conflates the sanitizer's intermediate output with the final MATCH string. At both the Issue's source revision `32513da690653a8baf30d2af4ae77d2129838f55` and implementation start `8d7b8dcfe2a0e1b487270474245de8fb2f6af11f`, `fts_expand_short_terms` quotes retained operator-like keywords: `foo OR bar` becomes `"foo" AND "OR" AND "bar"` with normalization disabled. It does not execute Boolean OR. This correction supersedes that note's operator-execution claim; dangling-operator removal and exclusion from vocab expansion remain unchanged. Issue #297 preserves this literal contract and changes the private intermediate representation to literal tokens, quoted only during final MATCH serialization. Input quote characters remain literal data and are escaped, not balanced or interpreted as phrase delimiters. See the current [FTS pipeline instructions](../../README.md#fts-クエリパイプライン) for the public contract and amici wire-format impact. The earlier audit remains historical evidence, not verification of the final MATCH semantics.
