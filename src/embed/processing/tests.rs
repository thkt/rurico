...
// Exercise the production planner with distinct word IDs, so dropped tails,
// wrong overlaps, prefixes and document order cannot hide behind repeated text.
#[test]
fn document_planning_preserves_prefix_overlap_tail_and_document_order() {
    ...
    let long = words.join(" ");
    let (chunks, counts) = plan_document_chunks(
        &tokenizer,
        &["語0", &long, ""],
        &prefix,
        max_content(prefix.len()),
    )
    .unwrap();
    ...
    // Second chunk starts 2048 content tokens before the first ends.
    ...
    // A candidate one token too long must shrink after prefix-aware encoding,
    // preserving EOS and the same content as the first planned chunk.
    let encoded = tokenizer.encode(long.as_str(), false).unwrap();
    let mut end = 8190;
    let shrunk =
        super::shrink_chunk_to_fit(&tokenizer, &long, encoded.get_offsets(), 0, &mut end).unwrap();
    assert_eq!(end, 8189);
    assert_eq!(shrunk, expected_first);
    let empty = plan_document_chunks(&tokenizer, &[], &prefix, max_content(prefix.len())).unwrap();
